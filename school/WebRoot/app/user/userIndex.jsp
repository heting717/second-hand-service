...
<html>
  <head>
	...
	<style type="text/css">
		...
		.message_lb{width: 800px;height: 300px;margin: 1px 200px;background-color:#F0F8FF;}
  ...
  <body>
	...
	<div class="hr_lb">
		<hr />
		<a href="${pageContext.request.contextPath}/UserIndexServlet?userId=${user.id}&method=showAllMessageList">出售中</a>&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/UserIndexServlet?userId=${user.id}&method=showAllBuyMessageList">他人想购</a>&nbsp;&nbsp;<a href="user0yuan.html">0元换购</a>
		<hr />
	</div>
			...
			<c:choose>
				<c:when test="${not empty requestScope.mainTypeList }">
					<c:forEach var="mainType" items="${requestScope.mainTypeList }">
							<tr><td><a href="${pageContext.request.contextPath}/UserIndexServlet?userId=${user.id}&mainTypeId=${mainType.id}&method=showTypeAndData">${mainType.mainTypeName}</a></td></tr>
					</c:forEach>
				</c:when>
			</c:choose>
		</table>
		<table align="right" class="table_lb">
			...
			<tr><td><a href="${pageContext.request.contextPath}/UserMessageServlet?userId=${user.id}&method=showInformation">已购买</a></td></tr>
				...
				<tr>
					<c:choose>
						<c:when test="${not empty requestScope.typeList}">
							...
							<td><a href="${pageContext.request.contextPath}/UserIndexServlet?userId=${user.id}&typeId=${type.id}&method=showAllMessageList">${type.typeName}</a></td>
							</c:forEach>
						</c:when>
					</c:choose>
				</tr>
			...
			<c:choose>
				<c:when test="${not empty requestScope.pb.pageData}">
					...
					<table align="left">
						...
						<tr>
							<td>
								<a href="${pageContext.request.contextPath}/UserIndexServlet?userId=${user.id}&sellMessageId=${sellMessage.id}&method=showSellMessage">${sellMessage.sellName}</a></td><td><a href="${pageContext.request.contextPath}/MessageServlet?userId=${user.id}&sellMessageId=${sellMessage.id}&method=showSellMessage">${sellMessage.brandName}</a> 
							</td>
						</tr>
