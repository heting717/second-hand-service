			...
			<c:choose>
				<c:when test="${not empty requestScope.mainTypeList }">
					<c:forEach var="mainType" items="${requestScope.mainTypeList }">
							<tr><td><a href="${pageContext.request.contextPath}/IndexServlet?mainTypeId=${mainType.id}&method=buyShowTypeAndData">${mainType.mainTypeName}</a></td></tr>
					</c:forEach>
				</c:when>
			</c:choose>
		</table>
		<div class="message_lb">
				...
				<tr>
					<c:choose>
						<c:when test="${not empty requestScope.typeList}">
							...
							<td><a href="${pageContext.request.contextPath}/IndexServlet?typeId=${type.id}&method=showAllBuyMessageList">${type.typeName}</a></td>
							</c:forEach>
						</c:when>
					</c:choose>
				</tr>
			...
			<c:choose>
				<c:when test="${not empty requestScope.pb.pageData}">
					...
					<table align="left">
						<tr>
							<td><a href="${pageContext.request.contextPath}/IndexServlet?buyMessageId=${buyMessage.id}&method=showBuyMessage">${buyMessage.brandName}</a></td>
						</tr>
